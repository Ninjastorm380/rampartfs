...
package() {
	...
	cp $srcdir/rampartfs/rampartfs/bin/Release/net9.0/linux-x64/publish/libMonoFuseHelper.so $pkgdir/usr/bin/libMonoFuseHelper.so
	cp $srcdir/rampartfs/rampartfs/bin/Release/net9.0/linux-x64/publish/libMonoPosixHelper.so $pkgdir/usr/bin/libMonoPosixHelper.so
}